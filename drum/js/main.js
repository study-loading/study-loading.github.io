(function () {
  ...
  function run(start, end) {
    return function (ripple, timing) {
      return function (params) {
        var tl = new TimelineMax();
        var realStart = Object.assign({}, start, params)
        tl.fromTo(ripple, timing, realStart, end);
        return tl;
      }
    }
  }
  ...
  var leftCircleScale = scale($('.circle-left'), 0.75)
  var rightCircleScale = scale($('.circle-right'), 0.75)

  $('.box-left').addEventListener('touchstart', function (e) {
    leftCircleScale({
      x: e.touches[0].pageX,
      y: e.touches[0].pageY
    })
  })
  $('.box-right').addEventListener('touchstart', function (e) {
    rightCircleScale({
      x: e.touches[0].pageX,
      y: e.touches[0].pageY
    })
